...
> **⚠️ This is a simple, naive project built for learning and experimentation. Not production-ready — no auth, runs on SQLite, no persistence guarantees. Use at your own risk.**

Maestro is a lightweight distributed job orchestration system. A central **server** queues jobs, registers **agents** (machines that execute work), and receives results. Each **agent** manages a pool of **workers** running in separate processes to process jobs in parallel.
...
This is a toy project — no authentication, single SQLite database, best-effort job delivery. It's meant for learning or lightweight personal use, not for anything serious.
